...
public class TransactionsDialog extends JDialog {
    ...
    public TransactionsDialog(Window owner, BankService service) {
        super(owner, "Transactions", ModalityType.APPLICATION_MODAL);
    ...
    private JPanel buildControls() {
        ...
        prev.addActionListener(e -> { if (offset > 0) { offset = Math.max(0, offset - pageSize); loadPage(); }});

        JButton next = new JButton("Next");
        next.addActionListener(e -> { if (lastFetched == pageSize) { offset += pageSize; loadPage(); }});
